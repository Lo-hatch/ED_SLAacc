module update_derived_utils
   ...
   ! cohort.                                                                               !
   !---------------------------------------------------------------------------------------!
   subroutine update_cohort_derived_props(co2,cpatch,ico,lsl,new_year,llspan_toc            &
      ...
      case (-1,1) ! Update trait every year
         if (new_year) call update_cohort_plastic_trait(co2,cpatch,ico,.false.                 &
                                                       ,llspan_toc,vm_bar_toc              &
                                                       ,rd_bar_toc,sla_toc)
      case (-2,2,3) ! Update trait every month
         call update_cohort_plastic_trait(co2,cpatch,ico,.false.                               &
   ...
   !=======================================================================================!
   !---------------------------------------------------------------------------------------!
   subroutine update_cohort_plastic_trait(co2,cpatch,ico,is_instant                            &
                                         ,llspan_toc,vm_bar_toc,rd_bar_toc,sla_toc)
      use ed_state_vars  , only : patchtype               ! ! structure
      ...
      implicit none
      !----- Arguments --------------------------------------------------------------------!
        real           , intent(in) :: co2      ! co2 of polygon
...
real                          :: sla_toc_new
      ...
      !------------------------------------------------------------------------------------!
      !-----------------------------luzihan 2022/12/7 --------------------------------!
       if (include_pft_sla(ipft)) then
  a = 0.865
b = -2.09
c = 0.00435
rel_sla_ref = 1.2153
 sla_toc_new = sla_toc / rel_sla_ref * a * (1-b * exp(-c * co2))
else
    sla_toc_new = sla_toc
    end if
      ...
      case ( 1, 2)
         ...
         new_sla = sla_toc_new * exp(lnexp)

         !---------------------------------------------------------------------------------!
      case (-1,-2)
         !------ SLA is defined at the bottom of canopy, use height to change SLA. --------!
         new_sla = sla_toc_new / (1. + lma_slope(ipft) * cpatch%hite(ico))
         !---------------------------------------------------------------------------------!
      case (3)
        !------------------------------------------------------------------------------------!
        ! Check the plasticity for vm0 above for details                                     !
        ...
        trait_change_frac  = sla_toc_new * exp(lnexp) / cpatch%sla(ico) - 1.
        instant_change_flag = (abs(trait_change_frac) < 0.05) .or. is_instant
        trait_change_frac = trait_change_frac                                          &
                          * merge(1., min(1.,1. / cpatch%llspan(ico)),instant_change_flag)
