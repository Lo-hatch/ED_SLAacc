...
subroutine load_ed_ecosystem_params()
   ...
   use pft_coms    , only : include_these_pft   & ! intent(in)
                          , include_these_pft_sla   & ! intent(in)
   ...
   do p=1,n_pft
      ...
      include_pft_sla    = .false.
   do p=1,n_pft
      if (include_these_pft_sla(p) >  0 .and. include_these_pft_sla(p) <= n_pft) then
         include_pft_sla(include_these_pft_sla(p)) = .true.
      end if
   end do
   !---------------------------------------------------------------------------------------!
